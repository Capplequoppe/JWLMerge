namespace JWLMergeCore.BackupFileServices.Models.DatabaseModels
{
    ...
    public class Database
    {
        ...
        public Database(ILogger? logger = null)
        {
            _logger = logger;
            ReinitializeIndexes();
            _notesGuidIndex = new Lazy<Dictionary<string, Note>>();
            _notesIdIndex = new Lazy<Dictionary<int, Note>>();
            _inputFieldsIndex = new Lazy<Dictionary<int, List<InputField>>>();
            _notesVerseIndex = new Lazy<Dictionary<BibleBookChapterAndVerse, List<Note>>>();
            _userMarksGuidIndex = new Lazy<Dictionary<string, UserMark>>();
            _userMarksIdIndex = new Lazy<Dictionary<int, UserMark>>();
            _userMarksLocationIdIndex = new Lazy<Dictionary<int, List<UserMark>>>();
            _locationsIdIndex = new Lazy<Dictionary<int, Location>>();
            _locationsValueIndex = new Lazy<Dictionary<string, Location>>();
            _locationsBibleChapterIndex = new Lazy<Dictionary<string, Location>>();
            _tagsNameIndex = new Lazy<Dictionary<TagTypeAndName, Tag>>();
            _tagsIdIndex = new Lazy<Dictionary<int, Tag>>();
            _tagMapLocationIndex = new Lazy<Dictionary<string, TagMap>>();
            _tagMapNoteIndex = new Lazy<Dictionary<string, TagMap>>();
            _blockRangesUserMarkIdIndex = new Lazy<Dictionary<int, List<BlockRange>>>();
            _bookmarksIndex = new Lazy<Dictionary<string, Bookmark>>();
        }
        ...
        public static string GetDateTimeUtcAsDbString(DateTime dateTime) => $"{dateTime:s}Z";
        ...
        public Note? FindNote(string noteGuid) => _notesGuidIndex.Value.TryGetValue(noteGuid, out var note) ? note : null;

        public Note? FindNote(int? noteId) => noteId is null? null: _notesIdIndex.Value.TryGetValue(noteId.Value, out var note) ? note : null;
        ...
        public UserMark? FindUserMark(string userMarkGuid) => _userMarksGuidIndex.Value.TryGetValue(userMarkGuid, out var userMark) ? userMark : null;

        public UserMark? FindUserMark(int? userMarkId) => userMarkId is null ? null : _userMarksIdIndex.Value.TryGetValue(userMarkId.Value, out var userMark) ? userMark : null;
        ...
        public Tag? FindTag(int? tagId) => tagId is null ? null : _tagsIdIndex.Value.TryGetValue(tagId.Value, out var tag) ? tag : null;
        ...
        public Location? FindLocation(int? locationId) => locationId is null ? null : _locationsIdIndex.Value.TryGetValue(locationId.Value, out var location) ? location : null;

        public InputField? FindInputField(int locationId, string textTag) => !_inputFieldsIndex.Value.TryGetValue(locationId, out var list) ? null : list.SingleOrDefault(x => x.TextTag?.Equals(textTag, StringComparison.OrdinalIgnoreCase) ?? false);
        ...
        public Location? FindLocationByValues(Location locationValues)
        {
            ...
            return _locationsValueIndex.Value.TryGetValue(GetLocationByValueKey(locationValues), out var location) ? location : null;
        }
        ...
        public IReadOnlyCollection<BlockRange>? FindBlockRanges(int userMarkId) => _blockRangesUserMarkIdIndex.Value.TryGetValue(userMarkId, out var ranges) ? ranges : null;

        public Bookmark? FindBookmark(int locationId, int publicationLocationId)
        {
            var key = GetBookmarkKey(locationId, publicationLocationId);
        ...
        private Dictionary<string, Note> NoteIndexValueFactory() => Notes?.ToDictionary(note => note.Guid) ?? new Dictionary<string, Note>();
        ...
        private Dictionary<string, UserMark> UserMarkIndexValueFactory() => UserMarks.ToDictionary(userMark => userMark.UserMarkGuid);
        ...
        private Dictionary<int, Location> LocationsIndexValueFactory() => Locations.ToDictionary(location => location.LocationId);
        ...
        private static string GetLocationByValueKey(Location location) => $"{location.KeySymbol}|{location.IssueTagNumber}|{location.MepsLanguage}|{location.Type}|{location.BookNumber ?? -1}|{location.ChapterNumber ?? -1}|{location.DocumentId ?? -1}|{location.Track ?? -1}";

        private static string GetLocationByBibleChapterKey(int bibleBookNumber, int chapterNumber, string bibleKeySymbol) => $"{bibleBookNumber}-{chapterNumber}-{bibleKeySymbol}";
        ...
        {
            foreach (var userMark in UserMarks.Where(userMark => FindLocation(userMark.LocationId) == null))
            {
        ...
        {
            //foreach (var tagMap in TagMaps.Where(tagMap => tagMap.NoteId != null))
            //{
            ...
            //    if (FindNote(tagMap.NoteId!.Value) is null)
            //    {
        ...
        private void CheckNoteValidity()
        {
            foreach (var note in Notes)
            {
                if (note.UserMarkId != null && FindUserMark(note.UserMarkId) is null)
                {
                    throw new BackupFileServicesException($"Could not find user mark Id for note {note.NoteId}");
                }

                if (note.LocationId != null && FindLocation(note.LocationId) is null)
                {
        ...
        {
            foreach (var bookmark in Bookmarks.Where(bookmark => FindLocation(bookmark.LocationId) == null ||
        ...
        {
            foreach (var range in BlockRanges.Where(range => FindUserMark(range.UserMarkId) == null))
            {
        ...
        private int FixupUserMarkValidity()
        {
            ...
            for (var n = UserMarks.Count - 1; n >= 0; --n)
            {
                ...
                if (FindLocation(userMark.LocationId) == null)
                {
                    ...
                    _logger?.LogDebug($"Removed invalid user mark {userMark.UserMarkId}");
                }
            }
        ...
        private int FixupTagMapValidity()
        {
            ...
            for (var n = TagMaps.Count - 1; n >= 0; --n)
            {
                    ...
                    (FindTag(tagMap.TagId) == null || FindNote(tagMap.NoteId.Value) == null))
                {
                    ...
                    _logger?.LogError($"Removed invalid tag map {tagMap.TagMapId}");
                }
            }
        ...
        private int FixupNoteValidity()
        {
            ...
            for (var n = Notes.Count - 1; n >= 0; --n)
            {
                ...
                if (note.UserMarkId != null && FindUserMark(note.UserMarkId.Value) == null)
                {
                    ...
                    _logger?.LogError($"Cleared invalid user mark ID for note {note.NoteId}");
                }
                ...
                if (note.LocationId != null && FindLocation(note.LocationId.Value) == null)
                {
                    ...
                    _logger?.LogError($"Cleared invalid location ID for note {note.NoteId}");
                }
            }
        ...
        private int FixupBookmarkValidity()
        {
            ...
            for (var n = Bookmarks.Count - 1; n >= 0; --n)
            {
                    ...
                    FindLocation(bookmark.PublicationLocationId) == null)
                {
                    ...
                    _logger?.LogError($"Removed invalid bookmark {bookmark.BookmarkId}");
                }
            }
        ...
        private int FixupBlockRangeValidity()
        {
            ...
            for (var n = BlockRanges.Count - 1; n >= 0; --n)
            {
                ...
                if (FindUserMark(range.UserMarkId) == null)
                {
                    ...
                    _logger?.LogError($"Removed invalid block range {range.BlockRangeId}");
                }
            }
