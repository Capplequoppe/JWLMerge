namespace JWLMergeCore.BackupFileServices.Helpers
{
    ...
    {
        public Merger(ILogger? logger = null)
        {
            _logger = logger;
            ProgressEvent += Merger_ProgressEvent;
        }

        private static void Merger_ProgressEvent(object? sender, ProgressEventArgs e)
        {
        }
        ...
        private void MergeBookmarks(Database source, Database destination)
        {
            ...
            foreach (var bookmark in source.Bookmarks)
            {
                ...
                else
                {
                    if (location1 == null)
                    {
                        _logger?.LogError($"Could not find location for bookmark {bookmark.BookmarkId}");
                    }

                    if (location2 == null)
                    {
                        _logger?.LogError($"Could not find publication location for bookmark {bookmark.BookmarkId}");
                    }
                }
            }
        }
        ...
        {
            var newBookmark = bookmark.Clone();
        ...
        private void MergeTagMap(Database source, Database destination)
        {
            ...
            foreach (var sourceTagMap in source.TagMaps)
            {
                ...
                if (sourceTagMap.LocationId != null)
                {
                    ...
                    if (id == 0)
                    {
                        ...
                        if (location != null) InsertLocation(location, destination);
                        id = _translatedLocationIds.GetTranslatedId(sourceTagMap.LocationId.Value);
                    }
            ...
            NormalizeTagMapPositions(destination.TagMaps);
        }

        private static void NormalizeTagMapPositions(IEnumerable<TagMap> entries)
        {
        ...
        private void MergeTags(Database source, Database destination)
        {
            ...
            foreach (var tag in source.Tags.Where(t=> t != null))
            {
        ...
        {
            if (_translatedLocationIds.GetTranslatedId(location.LocationId) != 0) return;
        ...
        {
            var newUserMark = userMark.Clone();
